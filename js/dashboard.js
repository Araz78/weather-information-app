...
document.getElementById('btn_search').addEventListener('click', ()=>{
    ...
    var options = {
        //method: 'POST',
        url: 'http://127.0.0.1:5000/weatherdetails',
        form: {
            country: get_country,
            region: get_region,
            city: get_city
    ...
    request(options).then(function(data) {
        ...
        document.getElementById('weather_det_uv').innerHTML = JSON.parse(data)['current']['uv']
        document.getElementById('weather_det_precip').innerHTML = JSON.parse(data)['current']['precip_in']
        document.getElementById('weather_det_tempf').innerHTML = JSON.parse(data)['current']['temp_f']
